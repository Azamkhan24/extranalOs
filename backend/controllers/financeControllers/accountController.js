...
const createAccount = async (req, res) => {
  ...
  try {
    ...
    const commonAccountData = {
      group: req.body.group, // common
      name: req.body.name, // common
      alias: req.body.alias, // common
      printName: req.body.printName, // common
      opening_balance: { // Corrected the structure
        type_of_Account: req.body.opening_balance.type_of_Account, // Access the type of account
        balance: req.body.opening_balance.balance // Access the balance
      }, // common
    };
...
// Modify Account Controller pending
// Modify Account Controller pending
  ...
  try {
    ...
    switch (req.body.group) {
      ...
      case 'Sundry Creditors':
        ...
        if ((req.body.constitutions_of_bussiness === "Private Limited Company" || req.body.constitutions_of_bussiness === 'Public Limited Company') && !req.body.cin) {
          return res.status(400).json({ message: 'CIN is required for Private and Public companies.' });
        }
        if (req.body.name && req.body.name.endsWith('LLP') && !req.body.llpin) {
          return res.status(400).json({ message: 'LLPIN is required for LLP companies.' });
        }
        ...
        commonAccountData.llpin = req.body.llpin;
        commonAccountData.attachments = {
          canceled_cheque: req.body.canceled_cheque,
          gstin_certificate: req.body.gstin_certificate,
          others: req.body.others,
        };
        break;
